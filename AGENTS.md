...
- Feature branches and a pull request are required for master — **except** for generated and
  append-only records, which may be committed straight to master: `TODO.md` (regenerated by
  `/pstatus`), `docs/project_log.md` (append-only session journal), and `docs/sync-log.md`
  (bookkeeping). Nothing is reviewable in a file that is derived or appended, so a PR for one is
  ceremony, not a gate.
- Everything else takes a PR. The line that matters is blast radius: kit files
  (`.claude/commands/*`, `install-kit.sh`, `utility/sync-labels.sh`, `templates/*`,
  `.markdownlint.jsonc`) are `overwrite`-installed into every downstream repo, so a bad one costs
  a cleanup in each. Source, config, and workflows likewise.
