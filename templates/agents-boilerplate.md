...
- Inline HTML is __not__ allowed. Long lines are fine.
- Rules live in `.markdownlint-cli2.jsonc` — rules, globs and ignores, one file, read by the editor, CLI, CI and agents alike. Emphasis is pinned: `*italic*` and `__bold__`, identical in every repo. Only committed files are linted; anything `.gitignore`d is generated or vendored, so its source is linted instead.
