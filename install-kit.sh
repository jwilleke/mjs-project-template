...
uses_markdownlint_v1() {   # can the v1 `markdownlint` binary even run here?
  # Ask whether the PACKAGE is installed, not whether the word appears somewhere.
  # markdownlint-cli is what provides the v1 binary; without it, nothing in the
  # repo can call it and .markdownlint.jsonc is dead weight.
  #
  # Grepping for the word was tried twice and was wrong both times. It matched a
  # workflow step named `markdownlint` and a comment mentioning "the markdownlint
  # step" in jwilleke/yourphr — labels and prose, not invocations — and concluded
  # the repo needed a config that nothing there reads.
  [ -f "$TARGET/package.json" ] || return 1
  grep -qE '"markdownlint-cli" *:' "$TARGET/package.json"
}
